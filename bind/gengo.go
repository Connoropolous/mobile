...
func (g *goGen) genFuncBody(o *types.Func, selectorLHS string) {
	...
	res := sig.Results()
	if res.Len() > 2 {
		g.errorf("too many results being assigned " + o.Name() + " " + selectorLHS)
		return
	}
	if res.Len() == 2 && !isErrorType(res.At(1).Type()) {
		g.errorf("there are two arguments and the second one must be of type error " + o.Name() + " " + selectorLHS)
		return
	}
	if res.Len() > 0 {
		for i := 0; i < res.Len(); i++ {
...
func (g *goGen) genPreamble() {
	...
	g.Printf("import (\n")
	g.Indent()
	g.Printf("_seq \"github.com/Connoropolous/mobile/bind/seq\"\n")
